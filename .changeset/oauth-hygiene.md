...
- The Express, Hono, and Next.js `streamableHttpHandler` validate the `Origin` header, as the MCP spec requires: requests carrying an `Origin` whose hostname matches neither the request's own host nor the new `allowedOrigins` option are rejected with a `403` (DNS rebinding / CSRF defense). Non-browser MCP clients send no `Origin` and are unaffected; cross-origin browser clients need `allowedOrigins`.
