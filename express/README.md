...
Requests carrying an `Origin` header are rejected with a `403` unless the origin's hostname is localhost-class (`localhost`, `127.0.0.1`, `[::1]`) or listed in `allowedOrigins` — the Origin validation the MCP spec requires, protecting browser-reachable servers against DNS rebinding. Non-browser MCP clients send no `Origin` header and are unaffected.
